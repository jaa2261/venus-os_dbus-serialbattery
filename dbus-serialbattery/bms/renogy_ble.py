...
class Renogy_Ble(Renogy):
    ...
    def __init__(self, port: Optional[str], baud: Optional[int], address: str):
        ...
        self.run = True
        self.bt_thread = threading.Thread(name="Renogy_Ble_Loop", target=self.background_loop, daemon=True)
    ...
    async def bt_main_loop(self):
        try:
            self.device = await BleakScanner.find_device_by_address(self.port, cb=dict(use_bdaddr=True))
            logger.info(f"Found device: {self.device.address} ({self.device.name})")
        except Exception:
    ...
    def test_connection(self):
        ...
        result = False
        logger.info("Test of Renogy_Ble at " + self.address)
        try:
            if self.address:
                result = True
    ...
    async def async_read_serial_data_renogy(self, command):
        if self.hci_uart_ok:
            try:
                cmd = self.generate_command(command)

                bt_task = asyncio.run_coroutine_threadsafe(self.send_command(cmd), self.bt_loop)
                result = await asyncio.wait_for(asyncio.wrap_future(bt_task), timeout=60)
                return result
            except asyncio.TimeoutError:
    ...
    def read_serial_data_renogy(self, command):
        if not self.bt_loop:
            return False
        try:
            data = asyncio.run(self.async_read_serial_data_renogy(command))
...
if __name__ == "__main__":
    bat = Renogy_Ble("Foo", -1, sys.argv[1])
