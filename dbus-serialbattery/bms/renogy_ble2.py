...
class Renogy_Ble2(Renogy):
    def __init__(self, port, baud, address):
        super(Renogy_Ble2, self).__init__(port, baud, address)
    ...
    def on_data_received(self, client, data):
        self.data = data
        logger.info(f"{client.ble_manager.device.name} => {data}")
        if len(self.data) == 0:
            logger.warning("Failed to get any data")
        client.stop()
    ...
    def test_connection(self):
        ...
        result = False
        try:
            battery = RenogyBLEBatteryClient(config, int(self.address, 16), self.on_data_received, self.on_error)
            battery.start()
    ...
    def get_settings(self):
        ...
        """
        if len(self.data) == 0:
            logger.error("Failed to get any data")
            return False
        ...
        self.cell_count = self.data.get("cell_count", 0)

        # capacity of the battery in ampere hours (float)
        self.capacity = self.data.get("capacity", 0)

        # OPTIONAL values to set
        # does not need to be in this function
        # could also be read in a function that is called from refresh_data()
        # maximum charge current in amps (float)
        self.max_battery_charge_current = self.data.get("charge_current_limit", 0)

        # maximum discharge current in amps (float)
        self.max_battery_discharge_current = self.data.get("discharge_current_limit", 0)
        ...
        self.max_battery_voltage_bms = self.data.get("charge_voltage_limit", 0)

        # minimum voltage of the battery in V (float)
        self.min_battery_voltage_bms = self.data.get("discharge_voltage_limit", 0)
    ...
    def read_status_data(self):
        ...
        self.voltage = self.data.get("voltage", 0)

        # current of the battery in amps (float)
        self.current = self.data.get("current", 0)

        # state of charge in percent (float)
        # self.soc =

        # temperature sensor 1 in °C (float)
        temperature_1 = self.data.get("temperature_0", 0)
        self.to_temperature(1, temperature_1)

        # status of the battery if charging is enabled (bool)
        self.charge_fet = self.data.get("status_1_values").get("charge_MOSFET", 0)

        # status of the battery if discharging is enabled (bool)
        self.discharge_fet = self.data.get("status_1_values").get("discharge_MOSFET", 0)
        """
        ...
        self.capacity_remaining = self.data.get("remaining_charge", 0)

        # temperature sensor 2 in °C (float)
        temperature_2 = self.data.get("temperature_1", 0)
        self.to_temperature(2, temperature_2)

        # temperature sensor 3 in °C (float)
        temperature_3 = self.data.get("temperature_2", 0)
        self.to_temperature(3, temperature_3)

        # temperature sensor 4 in °C (float)
        temperature_4 = self.data.get("temperature_3", 0)
        ...
        self.history.charge_cycles = self.data.get("cycle_count", 0)
    ...
    def read_cell_data(self):
        ...
        for c in range(self.cell_count):
            self.cells[c].voltage = self.data.get(f"cell_voltage_{c}", 0)
